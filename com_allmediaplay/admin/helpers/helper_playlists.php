...
require_once(JPATH_COMPONENT_ADMINISTRATOR.DS.'helpers'.DS.'helper_playlist.php');

class AllMediaPlayPlaylistsHelper {
    ...
    function &getPlaylists($dir) {
        ...
        foreach ($candidates as $playlist) {
            $helper = new AllMediaPlayPlaylistHelper($dir.DS.$playlist);
            if ($pl =& $helper->read()) {
                $obj = new stdClass();
